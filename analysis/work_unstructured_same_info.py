...
def _build_prompt(*, repair_input: Mapping[str, object]) -> str:
    # This is "same info" as call_patch repair_input, but with an unstructured output contract.
    payload = json.dumps(dict(repair_input), ensure_ascii=False, indent=2)
    return (
        "You are a WorkBench plan fixer.\n"
        "\n"
        "You are given REPAIR_INPUT_JSON. Use it fully.\n"
        "\n"
        "Rules:\n"
        "- Use ONLY tools listed in REPAIR_INPUT_JSON.tool_schemas.\n"
        "- When calling a tool, use ONLY its allowed param keys.\n"
        "- Ensure required params are non-empty.\n"
        "- Do NOT invent IDs: if you set event_id/email_id/task_id/customer_id/visitor_id, it MUST be in REPAIR_INPUT_JSON.candidate_ids.\n"
        "- When setting board/list_name/status/value_to_plot, use EXACTLY one of REPAIR_INPUT_JSON.canonical_values entries.\n"
        "- Prefer removing unrelated side-effect actions; only keep/add DO calls explicitly requested by the query.\n"
        "\n"
        "Output format (important):\n"
        "- Output ONLY a JSON array of strings.\n"
        "- Each string is one function call: tool.action.func(k=\"v\", ...)\n"
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="WorkBench ablation: same repair input as call_patch, but unstructured output (call list, not patch ops)."
    )
...
def _run_one(
    *,
    ...
    # --- Unstructured rewrite (same info, no patch ops) ---
    ...
    def _repair_one(pos: int, idx: int) -> Tuple[int, Dict[str, object], Optional[Dict[str, int]]]:
        ...
        if no_llm:
            ...
            prompt = _build_prompt(repair_input=repair_input)
            messages = [
    ...
    if token_counter.calls:
        lines.append(
            "LLM token usage (unstructured_same_info): "
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    if args.full:
        ...
        group_name = str(args.run_name).strip() or f"unstructured_same_info_full_{split}_{stamp}"
    ...
    if not run_name:
        run_name = f"unstructured_same_info_{_model_slug(args.model)}_{split}_{stamp}"
    run_dir = _run_one(
        split=split,
        base_model=base_model,
