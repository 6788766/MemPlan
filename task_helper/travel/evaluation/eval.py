...
def compute_all_budget_pass_rate(hard_constraint_record):
    budget_true = 0
    budget_total = 0

    for level_days in hard_constraint_record.values():
        for day_constraints in level_days.values():
            budget = day_constraints.get('valid_cost')
            if not budget:
                continue
            budget_true += int(budget.get('true', 0) or 0)
            total = budget.get('total')
            if total is None:
                total = int(budget.get('true', 0) or 0) + int(budget.get('false', 0) or 0)
            budget_total += int(total or 0)

    if budget_total == 0:
        return 0.0
    return budget_true / budget_total
...
def eval_score(set_type: str, file_path: str):
    ...
    result['All Budget Pass Rate'] = compute_all_budget_pass_rate(hardConstraint_statistic_processed)
